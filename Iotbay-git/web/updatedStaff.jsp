<%-- 
    Document   : updatedStaff
    Created on : 17/04/2022, 9:20:12 PM
    Author     : Aliza faisal
--%>


<%@page import="uts.isd.model.Staff"%>
<%@page import="uts.isd.controller.FetchSelectedStaff"%>
<%@page import="uts.isd.controller.UpdateStaff"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
              <%
            int id = Integer. parseInt(request.getParameter("id"));
            String fname = request.getParameter("updatedfname");
            String lname = request.getParameter("updatedlname");
            String address = request.getParameter("updatedaddress");
            String email = request.getParameter("updatedemail");
            String username = request.getParameter("updatedusername");
            String password = request.getParameter("updatedpassword");
            String position = request.getParameter("updatedposition");
            String mobile = request.getParameter("updatedmobile");
            Boolean staffStatus = true;
            UpdateStaff us = new UpdateStaff();
            String result = us.Update(us.getConnDB(),id, fname,lname, email, position, address, username, password, staffStatus, mobile);
          
              %>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Updated Staff</title>
          <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    </head>

    <body>
        <div class="alert alert-success" role="alert">
      <%= result %>
      </div>
      <a href="viewStaff.jsp" class="btn btn-primary">View Staff Members</a>
        

       </div>
       
    </body>
</html>

